...
test.describe('Quick Math Game', () => {
  ...
  for (let run = 1; run <= 3; run++) {
    test(`Run ${run}: Should load and play Quick Math without errors`, async ({ page }) => {
      const errors = await setupConsoleErrorCollector(page);

      // Navigate to Quick Math game
      await page.goto('/games/quick-math');
      await page.waitForLoadState('networkidle');
      await page.waitForTimeout(1000);

      // Check game loaded
      const titleOrContent = await page.locator('text=/Quick Math|Math|clock|START/i').first();
      ...
      const startButton = page.locator('text=/START|Play|Begin/i').first();
      if (await startButton.isVisible()) {
        ...
        // Try clicking an answer button
        const answerButtons = page.locator('[role="button"], [class*="answer"], [class*="option"]');
  ...
  test('Should display math problems', async ({ page }) => {
    const errors = await setupConsoleErrorCollector(page);
    ...
    const gameContent = page.locator('text=/Quick|Math|START|clock/i').first();
